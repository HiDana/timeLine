!(function() {
  "use strict";
  var Timesheet = function(container, min, max, data) {
    (this.data = []),
      (this.year = { min: min, max: max }),
      this.parse(data || []),
      "undefined" != typeof document &&
        ((this.container =
          "string" == typeof container
            ? document.querySelector("#" + container)
            : container),
        this.drawSections(),
        this.insertData());
  };
  (Timesheet.prototype.insertData = function() {
    for (
      var html = [],
        widthMonth = this.container.querySelector(".scale section").offsetWidth,
        n = 0,
        m = this.data.length;
      m > n;
      n++
    ) {
      // console.log("cur", this.data[n]);

      var cur = this.data[n],
        bubble = this.createBubble(
          widthMonth,
          this.year.min,
          cur.start,
          cur.end
        ),
        line = [
          '<span style="margin-left: ' +
            bubble.getStartOffset() +
            "px; width: " +
            bubble.getWidth() +
            'px;" class="bubble bubble-' +
            (cur.type || "default") +
            '" data-duration="' +
            (cur.end
              ? Math.round((cur.end - cur.start) / 1e3 / 60 / 60 / 24 / 39)
              : "") +
            '"></span>',
          '<span class="date">' + bubble.getDateLabel() + "</span> ",
          '<span class="label">' + cur.label + "</span>"
        ].join("");

      // console.log(line);

      html.push("<li>" + line + "</li>");
    }
    this.container.innerHTML += '<ul class="data">' + html.join("") + "</ul>";
  }),
    (Timesheet.prototype.drawSections = function() {
      for (var html = [], c = this.year.min; c <= this.year.max; c++)
        html.push("<section>" + c + "</section>");
      (this.container.className = "timesheet color-scheme-default"),
        (this.container.innerHTML =
          '<div class="scale">' + html.join("") + "</div>");
    }),
    (Timesheet.prototype.parseDate = function(date) {
      //把 date 轉換成 timeStamp 格式
      return (
        1 === date.split("/").length
          ? ((date = new Date(parseInt(date, 10), 0, 1)),
            (date.hasMonth = !1),
            (date.hasDate = !1))
          : date.split("/").length === 2
          ? ((date = date.split("/")),
            (date = new Date(parseInt(date[0], 10), parseInt(date[1], 10) - 1)),
            (date.hasMonth = !0),
            (date.hasDate = !1))
          : ((date = date.split("/")),
            (date = new Date(
              parseInt(date[0], 10),
              parseInt(date[1], 10) - 1,
              parseInt(date[2], 10) - 1
            )),
            (date.hasMonth = !0),
            (date.hasDate = !0)),
        date
      );
    }),
    (Timesheet.prototype.parse = function(data) {
      // console.log("data", data);

      //這邊主要是 parse data 成 start,end,label
      for (var n = 0, m = data.length; m > n; n++) {
        // console.log("data[n][0]", data[n][0]); //起始日期
        // console.log("data[n][1]", data[n][1]); //結束日期

        const toDay = new Date();
        toDay.hasMonth = !0;
        toDay.hasDate = !0;

        var beg = this.parseDate(data[n][0]),
          end = 4 === data[n].length ? this.parseDate(data[n][1]) : toDay,
          lbl = 4 === data[n].length ? data[n][2] : data[n][1],
          cat =
            4 === data[n].length
              ? data[n][3]
              : 3 === data[n].length
              ? data[n][2]
              : "default";

        // console.log("beg", beg);
        // console.log("end", end);
        // console.log("lbl", lbl);
        // console.log("cat", cat);

        beg.getFullYear() < this.year.min &&
          (this.year.min = beg.getFullYear()),
          end && end.getFullYear() > this.year.max
            ? (this.year.max = end.getFullYear())
            : beg.getFullYear() > this.year.max &&
              (this.year.max = beg.getFullYear()),
          this.data.push({ start: beg, end: end, label: lbl, type: cat });
        // console.log("parse date", this.data);
      }
    }),
    (Timesheet.prototype.createBubble = function(wMonth, min, start, end) {
      return new Bubble(wMonth, min, start, end);
    });
  var Bubble = function(wMonth, min, start, end) {
    (this.min = min),
      (this.start = start),
      (this.end = end),
      (this.widthMonth = wMonth);
  };
  (Bubble.prototype.formatMonth = function(num) {
    return (num = parseInt(num, 10)), num >= 10 ? num : "0" + num;
  }),
    (Bubble.prototype.getStartOffset = function() {
      return (
        (this.widthMonth / 12) *
        (12 * (this.start.getFullYear() - this.min) + this.start.getMonth())
      );
    }),
    (Bubble.prototype.getFullYears = function() {
      return (
        ((this.end && this.end.getFullYear()) || this.start.getFullYear()) -
        this.start.getFullYear()
      );
    }),
    (Bubble.prototype.getMonths = function() {
      var fullYears = this.getFullYears(),
        months = 0;
      return (
        this.end
          ? this.end.hasMonth
            ? ((months += this.end.getMonth() + 1),
              (months +=
                12 - (this.start.hasMonth ? this.start.getMonth() : 0)),
              (months += 12 * (fullYears - 1)))
            : ((months +=
                12 - (this.start.hasMonth ? this.start.getMonth() : 0)),
              (months += 12 * (fullYears - 1 > 0 ? fullYears - 1 : 0)))
          : (months += this.start.hasMonth ? 1 : 12),
        months
      );
    }),
    (Bubble.prototype.getWidth = function() {
      return (this.widthMonth / 12) * this.getMonths();
    }),
    (Bubble.prototype.getDateLabel = function() {
      return [
        `${this.start.getFullYear()}${
          this.start.hasMonth
            ? `/${this.formatMonth(this.start.getMonth() + 1)}`
            : ""
        }${
          this.start.hasDate
            ? `/${this.formatMonth(this.start.getDate() + 1)}`
            : ""
        }`,
        this.end
          ? `-${this.end.getFullYear()}${
              this.end.hasMonth
                ? `/${this.formatMonth(this.end.getMonth() + 1)}`
                : ""
            }${
              this.end.hasDate
                ? `/${this.formatMonth(this.end.getDate() + 1)}`
                : ""
            }`
          : ""
      ].join("");
    }),
    (window.Timesheet = Timesheet);
})();
